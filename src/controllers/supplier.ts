...
export const getNewSupplierCode = async(req: Request, res: Response): Promise<any> =>   {
    
    try {
        const prefixes = await Prefix
            .find();
        console.log(prefixes);
        let number = prefixes[0].nextSupplierCode;
        const result = await Prefix.findOneAndUpdate({ _id: prefixes[0]._id },
            {
                $set: {
        ...
        const UpdatedPrefixes = await Prefix
        .find();
        if(result){
...
export const getSupplier = async (req: Request, res: Response): Promise<any> => {
    try {
        const supplier = await Supplier.findOne({ supplierCode: req.params.supplierCode });
        if (supplier === null) {
            return res.status(400).json({
                success: false,
                data: {
...
export const createOrUpdateSupplier = async(req: Request, res: Response): Promise<any> =>  {
    try {
        const supplierExists = await Supplier.findOne({ supplierCode: req.body.supplierCode });
        if (supplierExists) return res.status(400).json({
            success: false, 
            data: {
                errorCode: 400,
...
export const updateSupplier = async(req: Request, res: Response): Promise<any> => {
    try {
        const supplier = await Supplier.findOneAndUpdate({ supplierCode: req.body.supplierCode }, req.body);
        if (supplier === null) {
            return res.status(400).json({
                success: false,
                data: {
            ...
            const updatedSupplier = { supplierCode: req.body.supplierCode, ...req.body };
...
export const deleteSupplier = async(req: Request, res: Response): Promise<any> => {
    try {
        const supplier = await Supplier.findOneAndDelete({ supplierId: req.params.supplierId });
        if (supplier === null) {
            return res.status(400).json({
                success: true,
                data: {
