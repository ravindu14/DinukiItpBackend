...
export const getNewEmployeeCode = async(req: Request, res: Response): Promise<any> =>   {
...
export const createOrUpdateEmployee = async(req: Request, res: Response): Promise<any> =>  {
    try {
        const userExists = await Employee.findOne({ employeeId: req.body.employeeId });
...
export const updateEmployee = async(req: Request, res: Response): Promise<any> => {
    try {
        const employee = await Employee.findOneAndUpdate({ employeeId: req.body.employeeId }, req.body);
        if (employee === null) {
            return res.status(400).json({
                success: false,
                data: {
