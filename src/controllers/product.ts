...
export const getNewProductCode = async(req: Request, res: Response): Promise<any> =>   {
...
export const createOrUpdateProduct = async(req: Request, res: Response): Promise<any> =>  {
    try {
        const userExists = await Product.findOne({ productCode: req.body.productCode });
...
export const updateProduct = async(req: Request, res: Response): Promise<any> => {
    try {
        const product = await Product.findOneAndUpdate({ productCode: req.body.productCode }, req.body);
        if (product === null) {
            return res.status(400).json({
                success: false,
                data: {
