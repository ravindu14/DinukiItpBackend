...
const getFilters = (filterOptions: IFilterOptions): ISelectFilterOptions => clearEmpties(removeEmpty({
...
export const getNewCustomerReturnCode = async(req: Request, res: Response): Promise<any> =>   {
...
export const getAllCustomerReturn = async(req: Request, res: Response): Promise<any> =>   {
    ...
    try {
        ...
        const customerReturnCount = await CustomerReturn.countDocuments({...constantFilters});
...
export const createOrUpdateCustomerReturn = async(req: Request, res: Response): Promise<any> =>  {
    try {
        const customerReturnExists = await CustomerReturn.findOne({ returnId: req.body.returnId });
        if (customerReturnExists) return res.status(400).json({
            success: false, 
            data: {
                errorCode: 400,
