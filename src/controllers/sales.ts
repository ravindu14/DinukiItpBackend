...
const getFilters = (filterOptions: IFilterOptions): ISelectFilterOptions => clearEmpties(removeEmpty({
    ...
    salesPersonId: filterOptions.salesPersonId? filterOptions.salesPersonId : null,
    date: filterOptions.date? {$eq: new Date(filterOptions.date)} : null
}));

export const getNewSalesCode = async(req: Request, res: Response): Promise<any> =>   {
...
export const createOrUpdateSales = async(req: Request, res: Response): Promise<any> =>  {
    try {
        const salesExists = await Sales.findOne({ saleId: req.body.saleId });
        if (salesExists) return res.status(400).json({
            success: false, 
            data: {
                errorCode: 400,
